...
def main():
    ...
    def fetch(item):
        ...
        if os.path.exists(fn) and os.path.getsize(fn) == sz and sha512(fn) == cs:
            return "skip", name
        for _ in range(4):
            # the seeded copy is from another snapshot, so its bytes differ:
            # `curl -C -` would resume from its length and splice the new
            # tail onto the old prefix, landing on the right size with the
            # wrong content — after which every retry sees a "complete" file
            # and does nothing. Start each attempt from an empty slot.
