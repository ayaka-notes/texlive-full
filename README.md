...
- (2026.2.2): We changes texlive mirror to `https://texlive.info` to align with Overleaf official image, also we use **daily-specific** tex live mirror to build image to avoid potential problem.
- (2026.2.1): We add a `tex` user to align with Overleaf's default compile image.
- (2026.1.21): Knitr has been added into the image, which can support R code compile in LaTeX document. 
